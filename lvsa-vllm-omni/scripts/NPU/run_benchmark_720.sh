...
run_benchmark() {
    ...
    if [ "$config_type" = "lvsa" ]; then
        ...
        export LVSA_PATCHES_PER_FRAME=3600 # for 720p
    ...
    for prompt in "${prompt_list[@]}"; do
        ...
        for frames in 81; do # 81 161 321 481
            ...
            for model in "Wan2.2-T2V-A14B-Diffusers"; do # Wan2.1-T2V-1.3B-Diffusers Wan2.2-T2V-A14B-Diffusers Wan2.2-TI2V-5B-Diffusers
                ...
                echo "Running ${prompt_id}_720p_${config_type}_${version}_${size}B_${frames}"
                args=(
                    ...
                    --fps 16
                    --seed 42
                    --flow-shift 5.0 # 12.0 for 480p, 5.0 for 720p
                    --boundary-ratio 0.875
                    --output ${prompt_id}_720p_${config_type}_${version}_${size}B_${frames}.mp4
                    ${additional_args}
                )
                python examples/offline_inference/text_to_video/text_to_video.py "${args[@]}" &> ${prompt_id}_720p_${config_type}_${version}_${size}B_${frames}.log
                grep "40/40" ${prompt_id}_720p_${config_type}_${version}_${size}B_${frames}.log
                grep "generation time" ${prompt_id}_720p_${config_type}_${version}_${size}B_${frames}.log
            done
        done
    done
}
